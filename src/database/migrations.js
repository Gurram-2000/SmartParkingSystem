...
const runMigrations = async () => {
  ...
  try {
    ...
    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_parking_spot_occupied ON parking_spots(is_occupied);
    `);

    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_parking_spot_vehicle_type ON parking_spots(vehicle_type);
    `);

    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_parking_spot_floor ON parking_spots(floor);
    `);

    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_transaction_vehicle_id ON parking_transactions(vehicle_id);
    `);

    await client.query(`
      CREATE INDEX IF NOT EXISTS idx_transaction_status ON parking_transactions(status);
    `);

    logger.info('Database migrations completed successfully');

    // Seed initial parking spots if not already seeded
    const spotsCount = await client.query(`
      SELECT COUNT(*) as count FROM parking_spots;
    `);

    if (spotsCount.rows[0].count === 0) {
      await seedParkingSpots(client);
      logger.info('Parking spots seeded successfully');
    }
...
const seedParkingSpots = async (client) => {
  const floors = 5;
